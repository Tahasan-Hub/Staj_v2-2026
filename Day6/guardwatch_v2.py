...
parser.add_argument("--kaynak",type=str,default="0",help="Video dosyasının yolu ve kamera ID'si")
parser.add_argument("--kayit",action="store_true",help="İhlalleri CSV dosyasına kaydetmeyi aktif eder.")
...
if kayit_aktif_mi:
    if not os.path.exists(csv_dosya):
        ...
        print("[BİLGİ] 'ihlaller.csv' dosyası başlıklarla birlikte sıfırdan oluşturuldu.")
    else:
        print("[BİLGİ] Mevcut 'ihlaller.csv' dosyası bulundu,kayıtlara buradan devam edilecek.")
else:
    print("[BİLGİ] '--kayit' komutu verilmedi.İhlaller sadece ekranda görünecek,CSV'ye kaydedilmeyecek.")                
def ayarları_yukle(dosya_yolu = "config.json"):
    ...
    if not os.path.exists(dosya_yolu): #os.path.exists Dosya fiziksel olarak orada mı bakıyor
        ...
        with open(dosya_yolu,"r") as f:
            ...
            for anahtar in gerekli_anahtarlar:
                if anahtar not in yuklenen_ayarlar:
                    print(f"HATA: Config dosyasında '{anahtar}' ayarı yazılmamış/silinmiş!")
                    eksik_var_mi = True
            if eksik_var_mi:
                print("Lütfen yukarıdaki eksik ayarları config.json dosyasına ekleyin.")
                sys.exit(1) #Eksik ayarlarla program çalışırsa ileride çöker.O yüzden şuan kapatıyoruz.
...
try:
    ...
    print("1. 'yolov8n.pt' dosyasının projenin olduğu klasörde bulunduğundan emin olun.")
...
if not kamera.isOpened(): #Kamera başarıyla açıldı mı diye kontrol ediyoruz
        ...
        if video_kaynagi == "0":
                print("İPUCU: Webcam takılı mı? Başka bir program (Zoom,Discord) kamerayı kullanıyor olabilir.")
        sys.exit(1)    
...
def eski_kayitlari_temizle(klasor_yolu = "kayitlar/2026-03-06",gun_limit = 7):
    ...
    if not os.path.exists(klasor_yolu): #Program ilk kez çalışıyorsa "kayitlar" klasörü hiç yoktur.Hata vermemesi için return ile fonksiyonu anında bitirip geri döndürüyoruz
        print("[BİLGİ] Kayıtlar klasörü henüz oluşturulmamış,temizlik atlandı.")
        return
    simdi = time.time()
    silinen_klasor_sayisi = 0

    for klasör_adi in os.listdir(klasor_yolu): # "kayitlar" klasörünün içindeki herşeye sırayla bak
        ...
        if os.path.isdir(tam_yol): #Sadece klasörlerle ilgilen
            try:
                ...
                if yas_gun > gun_limit:
                    shutil.rmtree(tam_yol) #Klasörü içindeki yüzlerce fotoğrafla beraber tek hamlede siler
                    silinen_klasor_sayisi += 1
                    print(f"SİLİNDİ: {klasör_adi} klasörü ({yas_gun:.1f} günlük çöp temizlendi.)")
            except ValueError:
                continue
    if silinen_klasor_sayisi > 0:
        print(f"[BİLGİ] GuardWatch Temizliği tamamlandı. {silinen_klasor_sayisi} adet eski klasör imha edildi.")                
...
def ihlal_frame_kaydet(frame,kisi_id,ear_degeri,sure_sn = 0.0):
    ...
    klasor_yolu = os.path.join("kayitlar",klasor_adi) #Yolu birleştirip klasörü oluşturuyoruz
    ...
    t_kayit = threading.Thread(target=resim_kaydet_thread,args=(frame.copy(),tam_kayit_yolu))
    t_kayit.start()
    ...
    try:
        with open(dosya_adi, "r", newline="") as f:
            ...
            for satir in okuyucu:
                ...
                if sure > en_uzun_ihlal: en_uzun_ihlal = sure
        ...
        print(f"HATA: '{dosya_adi}' dosyası bulunamadı.Henüz ihlal yapılmamış olabilir.")      
...
def grafikleri_ciz(dosya_adi):
    ...
    try:
        ...
        if len(sirali_saatler) > 0:
            ...
            plt.plot(sirali_saatler,saat_ihlal_sayilari,marker="o",color="crimson",linewidth=2)
...
def ihlal_kaydet(kisi_id ,ihlal_turu,sure,ear,frame = None):#Kişi uyuduğunda yada hareketsiz kaldığında bu fonksiyon çağrılır
# Gelen verileri alıp 'ihlaller.csv' dosyasının en alt satırına ekler.
    if not kayit_aktif_mi:
        return
    zaman = datetime.now().strftime("%Y-%m-%d %H:%M:%S") #Bugünün saati ve tarihini aldık
    frame_yolu = "" 

    if frame is not None: #Eğer fonksiyona bir fotoğraf (frame) gönderildiyse
        frame_yolu = ihlal_frame_kaydet(frame,kisi_id,ear,sure) #Fotoğrafı kaydet ve geri dönen dosya adresini 'frame_yolu' değişkenine eşitle

    with open("ihlaller.csv","a",newline="") as dosya: #Tüm verileri (fotoğraf yolu da dahil) CSV'ye tek seferde yaz
        yaz = csv.writer(dosya)
...
def kare_isle(frame):#Bir fotoğrafı alır,işler,üzerine çizim yapıp geri döndürür.
    ...
    try:
        ...
        for k_id,k_merkez in aktif_kisiler.items():#Ekranda aktif olan her bir kişi için çizim yapmaya başlıyoruz
            ...
            if ear_degeri is not None: # Eğer MediaPipe bir yüz veya göz bulabildiyse hesaplamaya başla
                    ...
                    if k_id not in uyku_zamanlayicilari:#Bu kişi hafızada yoksa yani gözünü ilk defa tam şuanda kapattıysa
                        ...
                        if gecen_sure > CONFIG["goz_kapali_limit_sn"]: #Eğer geçen süre configde ki limitten (2.0 sn) büyükse
                            ...
                            if loglanmis_ihlaller.get(f"{k_id}_uyku") != True: #Eğer bu adamın uykusunu daha önce dosyaya YAZMADIYSAK
                                ...
                                ihlal_kaydet(k_id,"goz_kapali",gecen_sure,ear_degeri,frame)
                            if gecen_sure >= 10.0 and loglanmis_ihlaller.get(f"{k_id}_uyku_arsiv") != True:
                                print(f"[KRİTİK UYARI] ID={k_id} 10 saniyeden uzun süredir uyuyor! Arşive kanıt alınıyor.")
                                ihlal_kaydet(k_id,"kritik_uyku",gecen_sure,ear_degeri,frame)
                ...
                if k_id in son_bilinen_konumlar:
                    hareket_miktari = oklid_hesapla(takip_noktasi,son_bilinen_konumlar[k_id])
                        ...
                        if k_id not in hareketsizlik_zamanlayicilari:
                            ...
                            if gecen_hareketsiz_sure > CONFIG["hareketsizlik_limit_sn"]:
                                ...
                                if loglanmis_ihlaller.get(f"{k_id}_hareketsiz") != True:
                                    ...
                                    guvenli_ear = ear_degeri if ear_degeri is not None else 0.0
                                    ihlal_kaydet(k_id,"hareketsiz",gecen_hareketsiz_sure,guvenli_ear,frame)
                                if gecen_hareketsiz_sure >= 10.0 and loglanmis_ihlaller.get(f"{k_id}_hareketsiz_arsiv") != True:
                                    print(f"[KRİTİK UYARI] ID={k_id} 10 saniyeden uzun süredir hareketsiz! Arşive kanıt alınıyor.")
                                    guvenli_ear = ear_degeri if ear_degeri is not None else 0.0
                                    ihlal_kaydet(k_id,"kritik_hareketsiz",gecen_hareketsiz_sure,guvenli_ear,frame)
                                    loglanmis_ihlaller[f"{k_id}_hareketsiz_arsiv"] = True
                    else:
            ...
            elif hareket_tehlikede == True and ear_degeri is not None:
        ...
        if genel_tehlike_durumu == True and ear_degeri is not None: #Bütün kişiler kontrol edildikten sonra bayrağın son durumuna bakıyoruz
           if not alarm_caliniyor:
               t_alarm = threading.Thread(target=alarm_cal,daemon=True) #Daemon True ise na program yani kamera aniden kapanırsa bunlarda kapancak işi bırakcak
               t_alarm.start() 
    except Exception as e:                 
...
def uyku_durumu_bul(rgb_frame):#MediaPipe ile göz noktalarını bulur ve EAR değerini hesaplayıp döndürür
    ...
    return ear_degeri ,burun_merkezi #Hesaplanan EAR değerini ve burnun nerede olduğunu geri gönderiyoruz    
...
if __name__ == "__main__": #Eğer bu dosya (guardwatch.py) doğrudan çalıştırıldıysa main() fonksiyonunu tetikle
    try:
        eski_kayitlari_temizle(gun_limit=7) #Kamera açılmadan hemen önce eski klasörleri limite göre temizle!
