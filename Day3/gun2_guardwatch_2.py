...
if not os.path.exists(csv_dosya):
    with open(csv_dosya , "w" , newline="") as dosya:
        ...
        yaz.writerow(["zaman","kisi_id","ihlal_turu","sure_sn","ear_degeri"])
...
def ihlal_kaydet(kisi_id ,ihlal_turu,sure,ear):#Kişi uyuduğunda yada hareketsiz kaldığında bu fonksiyon çağrılır
# Gelen verileri alıp 'ihlaller.csv' dosyasının en alt satırına ekler.
    ham_ts = time.time()
    zaman_objesi = datetime.fromtimestamp(ham_ts)
    zaman_formatli = zaman_objesi.strftime("%Y-%m-%d %H:%M:%S")
    with open("ihlaller.csv","a",newline="") as dosya: #Dosyayı "a"(Apend) yani ekleme modunda açtık.Eski veriler güvende!
        yaz = csv.writer(dosya)
        zaman = datetime.now().strftime("%Y-%m-%d %H:%M:%S") # Yıl-Ay-Gün Saat:Dakika:Saniye gibi bir metine çevirdik
        sure_metni = f"{sure:.2f}"
        ear_metni = f"{ear:.3f}" if ear is not None else "0.000"
        yaz.writerow([zaman_formatli,kisi_id,ihlal_turu,sure_metni,ear_metni])
                        ...
                        if k_id not in hareketsizlik_zamanlayicilari:
                            ...
                            if gecen_hareketsiz_sure > CONFIG["hareketsizlik_limit_sn"]:
                                ...
                                if loglanmis_ihlaller.get(f"{k_id}_hareketsiz") != True:
                                    ...
                                    ihlal_kaydet(k_id,"hareketsiz",gecen_hareketsiz_sure,ear_degeri)
                    else:
